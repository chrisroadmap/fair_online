...
### 6. User-uploadable custom emissions CSV — TODO, attempted and reverted 2026-09-02
...
- Parsed trajectory is round-tripped to the client and replayed in the `/api/run` body (no server-side session — this app is stateless, so a short-lived token would be the odd one out) and substituted for the preset's base emissions in `run_scenario`, upstream of the existing 2023-onward manual-edit control points (which still apply on top, if the user also drags sliders after uploading).
- Frontend: a file input in the "Custom emissions" section, next to the existing per-species edit controls.

**Bug found on first real-world test, implementation reverted (uncommitted, never shipped):** an uploaded CSV starting later than `YEAR_START` (e.g. 2020) silently wiped out real history all the way back to 1750. Cause: `run_scenario` substituted the uploaded trajectory as the species' entire base series over `_EMIS_YEARS` (1750–2100) via `np.interp(_EMIS_YEARS, upload_years, upload_values)` — and `np.interp` extrapolates *flat* outside the given x-range, so every year before the upload's own first year got clamped to that first value (a 2020-starting file made 1750–2020 a flat line at the 2020 number). The existing manual-edit path (`_apply_emissions_override`) gets this right by design: it always splices the user's trajectory onto the *preset's own historical values* from `EDIT_ANCHOR_YEAR` (2023) onward, never touching pre-2023 history. The upload path needs the same treatment — splice the uploaded trajectory onto the preset's real historical base from `EDIT_ANCHOR_YEAR` onward (or from `max(EDIT_ANCHOR_YEAR, upload's own first year)` if the upload starts later than 2023), not treat the upload as a full-range replacement. Re-derive `_apply_emissions_override`-style splicing for the upload case rather than a raw `np.interp` substitution, and add a test case for a late-starting upload (e.g. first year 2020 or 2030) confirming pre-upload history is untouched.
